-- Phase 15 cleanup: Remove phantom legacy signature and ensure only Phase 15 function exists.
--
-- ROOT CAUSE: Remote DB had a legacy function:
--   create_manual_order_rpc(p_created_by UUID, p_event_id UUID, p_participants JSONB, p_ticket_selections JSONB)
-- This was never dropped by previous migrations because its signature didn't match any DROP statements.
-- PostgREST only exposed this legacy function (404 for new params → HTTP 500).
--
-- FIX: Drop ALL possible overloads, then CREATE the correct Phase 15 function.

-- ============================================================
-- STEP 1: DROP all possible legacy/phantom overloads
-- ============================================================

-- Legacy 4-param (the phantom): (uuid, uuid, jsonb, jsonb)
DROP FUNCTION IF EXISTS public.create_manual_order_rpc(uuid, uuid, jsonb, jsonb);

-- Legacy 5-param from original migration 20260816000006: (uuid, jsonb, jsonb, payment_method, uuid)
DROP FUNCTION IF EXISTS public.create_manual_order_rpc(uuid, jsonb, jsonb, public.payment_method, uuid);

-- Fix migration 20260817000001 attempted signature: (uuid, jsonb, jsonb, uuid, payment_method)
DROP FUNCTION IF EXISTS public.create_manual_order_rpc(uuid, jsonb, jsonb, uuid, public.payment_method);

-- Any remaining with 4 params (uuid, jsonb, jsonb, uuid)
DROP FUNCTION IF EXISTS public.create_manual_order_rpc(uuid, jsonb, jsonb, uuid);

-- ============================================================
-- STEP 2: CREATE the correct Phase 15 function (single canonical signature)
-- ============================================================
...
-- ============================================================
-- STEP 3: Permissions — only service_role can call this function
-- ============================================================
...
COMMENT ON FUNCTION public.create_manual_order_rpc(UUID, JSONB, JSONB, UUID, public.payment_method) IS 'Phase 15: Atomic walk-in order. Creates order (MANUAL/APPROVED), payment (PAID), participants, order_items, consumes reservations, issues tickets, queues email jobs. No referral. No private-ticket admin override.';
