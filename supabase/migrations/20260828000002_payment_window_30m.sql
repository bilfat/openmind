-- ============================================================
-- Payment window: 1 hour -> 30 minutes
-- 1. Quota reservations now last 30 minutes (payment window).
-- 2. cleanup_expired_orders_rpc expires stale DRAFT/PENDING_PAYMENT
--    orders older than 30 minutes.
-- 3. pg_cron job re-scheduled with explicit 30-minute argument.
-- 4. Existing active reservations shortened to 30 minutes so new
--    window applies immediately.
-- ============================================================

-- ------------------------------------------------------------
-- 1. Quota reservation: default 60 minutes -> 30 minutes
-- ------------------------------------------------------------
...
--    Changed: p_stale_hours -> p_stale_minutes for 30-minute precision.
--    Note: parameter renamed, so DROP before re-create (PostgreSQL
--    disallows renaming args via CREATE OR REPLACE).
-- ------------------------------------------------------------
DROP FUNCTION IF EXISTS public.cleanup_expired_orders_rpc(INTEGER);

CREATE OR REPLACE FUNCTION public.cleanup_expired_orders_rpc(p_stale_minutes INTEGER DEFAULT 30)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_catalog
...
BEGIN
    ...
    WITH stale AS (
        ...
        WHERE status IN ('DRAFT'::public.order_status, 'PENDING_PAYMENT'::public.order_status)
          AND created_at < now() - (p_stale_minutes * INTERVAL '1 minute')
        RETURNING id
    )
...
-- ------------------------------------------------------------
-- 3. Re-schedule cleanup every 10 minutes with pg_cron,
--    passing the 30-minute window explicitly.
--    (More frequent than before to match the shorter window.)
-- ------------------------------------------------------------
...
--    the 30-minute rule applies immediately to existing data.
--    Orders 30-60 min old WILL expire on next cron run.
-- ------------------------------------------------------------
UPDATE public.ticket_reservations
SET reserved_until = now() + (30 * INTERVAL '1 minute')
WHERE status = 'RESERVED'::public.reservation_status
  AND reserved_until > now() + (30 * INTERVAL '1 minute');
