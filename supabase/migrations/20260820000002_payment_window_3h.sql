-- ============================================================
-- Payment window: 12 hours -> 3 hours
-- 1. Quota reservations now last 3 hours (payment window).
-- 2. cleanup_expired_orders_rpc expires stale DRAFT/PENDING_PAYMENT
--    orders older than 3 hours.
-- 3. pg_cron job is re-scheduled with an explicit 3-hour argument.
-- 4. Existing active reservations older than 3h are shortened so the
--    new window applies immediately (test data is safe to adjust).
-- ============================================================

-- ------------------------------------------------------------
-- 1. Quota reservation: 12 hours -> 3 hours (payment window)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.cleanup_expired_orders_rpc(p_stale_hours INTEGER DEFAULT 3)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_catalog
...
--    passing the 3-hour window explicitly.
-- ------------------------------------------------------------
...
SELECT cron.schedule(
    'openmind-cleanup',
    '*/15 * * * *',
    $$SELECT public.cleanup_expired_orders_rpc(3);$$
);

-- ------------------------------------------------------------
-- 4. Shorten active reservations older than the new window so
--    the 3-hour rule applies immediately to existing test data.
-- ------------------------------------------------------------
UPDATE public.ticket_reservations
SET reserved_until = now() + (3 * INTERVAL '1 hour')
WHERE status = 'RESERVED'::public.reservation_status
  AND reserved_until > now() + (3 * INTERVAL '1 hour');