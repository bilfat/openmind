-- =====================================================
-- Staff Role Migration
-- 1. Add STAFF to role_type enum
-- 2. Allow STAFF to create walk-in (MANUAL) orders
-- =====================================================

-- 1. New role: STAFF (check-in + walk-in sales only)
ALTER TYPE role_type ADD VALUE IF NOT EXISTS 'STAFF';

-- 2. Extend create_manual_order_rpc so an ACTIVE STAFF member can create
--    walk-in orders (same atomic flow, created_by records the staff).
...
BEGIN
  -- STEP 1: Validate operator identity (ADMIN / SUPER_ADMIN / STAFF)
  SELECT role, status INTO v_admin_role, v_admin_status
  FROM public.profiles WHERE id = p_admin_id;

  IF v_admin_role IS NULL OR v_admin_status != 'ACTIVE' OR v_admin_role NOT IN ('ADMIN', 'SUPER_ADMIN', 'STAFF') THEN
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'referral_code', NULLIF(trim(p_referral_code), '')
    )
  );
