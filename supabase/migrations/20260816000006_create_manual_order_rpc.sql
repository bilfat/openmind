-- Phase 15: Atomic walk-in / manual order creation for admin cash register.
-- Creates order, payment, participants, order_items, consumes reservations,
-- issues tickets, and queues email jobs — all within a single PL/pgSQL transaction.
-- Does NOT implement referral validation (Phase 16) or frontend integration (INT-006).
...
BEGIN
  ...
  -- STEP 12: Create Order Items (Model B)
  FOR v_selection IN SELECT * FROM jsonb_array_elements(p_ticket_selections)
  LOOP
  ...
  -- STEP 14: Issue Tickets via existing issuance RPC
...
REVOKE EXECUTE ON FUNCTION public.create_manual_order_rpc(UUID, JSONB, JSONB, public.payment_method, UUID) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_manual_order_rpc(UUID, JSONB, JSONB, public.payment_method, UUID) TO service_role;

COMMENT ON FUNCTION public.create_manual_order_rpc(UUID, JSONB, JSONB, public.payment_method, UUID) IS 'Phase 15: Atomic walk-in order. Creates order (MANUAL/APPROVED), payment (PAID), participants, order_items, consumes reservations, issues tickets, queues email jobs. No referral. No private-ticket admin override.';