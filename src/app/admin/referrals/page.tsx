...
export default function AdminReferralsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const data = await getStoredReferrals();
      if (!cancelled) {
        setReferrals(data);
        setLoading(false);
      }
  ...
  const handleToggleActive = async (ref: ReferralCode) => {
    if (ref.status === "INACTIVE") {
      await setReferralStatus(ref.id, "ACTIVE");
      showToast(`Kode referal "${ref.code}" telah diaktifkan kembali.`);
    } else {
      await setReferralStatus(ref.id, "INACTIVE");
  ...
  const handleArchive = async (ref: ReferralCode) => {
    await archiveReferral(ref.id);
    refreshList();
      ...
      <div className="rounded-3xl border border-border bg-white shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-16 gap-2 text-sm text-muted-foreground">
            ...
            <span>Memuat data kode referal...</span>
          </div>
        ) : (
